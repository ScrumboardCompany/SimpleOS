...
void Terminal::new_line() {
	if (pos / WIDTH >= HEIGHT) {
		...
		// Очищаем последнюю строку, заполняя ее нулевыми значениями
		for (size_t j = 0; j < WIDTH; ++j) {
			buffer[(HEIGHT - 1) * WIDTH * 2 + j * 2] = 0;  // Заменяем пробел на нулевое значение
		}

		// Устанавливаем позицию на начало последней строки
		set_pos((HEIGHT - 1) * WIDTH);
		set_buffer_pos(0);
	}
	else {
